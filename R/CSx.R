...
run_CSx <- function(bulk, reference, cibersortx_email, cibersortx_token){
  ...
  utils::write.table(sc_ref, file = "SignCSx.txt", sep = "\t", row.names = FALSE, quote = FALSE)

  bulk_ <- cbind.data.frame(Gene=rownames(bulk),bulk)
  utils::write.table(bulk_, file = "bulk.txt", sep = "\t", row.names = FALSE, quote = FALSE)
  ...
  if(code == 0){
    out_CSx <- utils::read.delim2(paste0(out_dir, "/CIBERSORTx_Results.txt"), row.names = 1)
