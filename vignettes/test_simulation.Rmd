...
reference = refDataIncomplet
bulk = as.matrix(simulation$bulk)
methodDeconv = "abis"
W_prime = 0
p_prime = 0
...
set.seed(2101)
refDataIncomplet <- reference[,-cellTypeOut$`30%`]
pathSave <- '/home/kba/Documents'
result_OPT <- DICEPRO::DICEPRO(reference = refDataIncomplet,
                               bulk = as.matrix(simulation$bulk),
                               methodDeconv = "abis",
                               hp_max_evals = 100,
                               output_path = pathSave,
                               hspaceTechniqueChoose = 'all')
paths <- generate_experiment_paths(pathSave, bulkName, refName,
                                     hspaceTechniqueChoose, algo_select)

file_name <- paste0(paths$data_dir, "/result_OPT_", bulkName, "_", refName, ".RData")
save(result_OPT, file = file_name)
cat("Objet sauvegardé dans :", file_name, "\n")
```
